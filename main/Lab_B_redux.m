...
f = waitbar(0,'Initialising','Name','Running iterations',...
...
for i = 1:iterations
    ...
    % calculate fitness
    ranks = samples - optimizeControlSystem(P(:,3:12),goals,priorities); % substracted from samples to invert the output to <samples> max, 0 min.
    crowdings = crowding(P,ranks);
    % create mating pool
    selectThese = btwr(ranks,crowdings,samples);
    % reject to half of population
